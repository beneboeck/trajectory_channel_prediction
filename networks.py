...
class my_VAE(nn.Module):
    ...
    def decode(self,z):
        ...
        if self.cov_type == 'DFT':
            ...
            log_pre2[log_pre < torch.log(torch.tensor(10e-4)).to(self.device)] = torch.log(torch.tensor(10e-4)).to(self.device)
            log_pre2[log_pre > torch.log(torch.tensor(1)).to(self.device)] = torch.log(torch.tensor(1)).to(self.device)
