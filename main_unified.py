...
if MODEL_TYPE == 'Trajectory':
    ...
    LD, memory, rnn_bool, en_layer, en_width, pr_layer, pr_width, de_layer, de_width, cov_type, BN, prepro, n_conv, cnn_bool, LB_var_dec, UB_var_dec, reg_output_var = 40,3,False,3,4,3,3,5,8,'DFT',False,'DFT',2,True,0.0084,0.6160,True
    rnn_bool = False
...
H_train = np.zeros((100000,2,32,SNAPSHOTS))
H_train[:,0,:,:] = np.real(H_train_c)
H_train[:,1,:,:] = np.imag(H_train_c)

H_val = np.zeros((10000,2,32,SNAPSHOTS))
H_val[:,0,:,:] = np.real(H_val_c)
H_val[:,1,:,:] = np.imag(H_val_c)

H_test = np.zeros((10000,2,32,SNAPSHOTS))
